...
((factory) => {
  ...
  function validationInfo(form) {
    const $form = $(form);
    let result = $form.data(data_validation);
    const onResetProxy = $.proxy(onReset, form);
    const defaultOptions = $jQval.unobtrusive.options || {};
    const execInContext = (name, args) => {
  ...
  $jQval.unobtrusive = {
    ...
    parse: (selector) => {
      ...
      const $forms = $selector.parents().addBack().filter('form').add($selector.find('form')).has('[data-val=true]');
  ...
  adapters.addMinMax = function (adapterName, minRuleName, maxRuleName, minMaxRuleName, minAttribute, maxAttribute) {
    ...
    /// <returns type="jQuery.validator.unobtrusive.adapters" />
    return this.add(adapterName, [minAttribute || 'min', maxAttribute || 'max'], (options) => {
  ...
  adapters.addSingleVal = function (adapterName, attribute, ruleName) {
    ...
    /// of adapterName will be used instead.</param>
    /// <returns type="jQuery.validator.unobtrusive.adapters" />
    return this.add(adapterName, [attribute || 'val'], (options) => {
      setValidationValues(options, ruleName || adapterName, options.params[attribute]);
    });
  };

  $jQval.addMethod('__dummy__', (value, element, params) => true);
  ...
  $jQval.addMethod('nonalphamin', (value, element, nonalphamin) => {
    let match;
    if (nonalphamin) {
  ...
  adapters.add('equalto', ['other'], (options) => {
  ...
  adapters.add('required', (options) => {
    // jQuery Validate equates "required" with "mandatory" for checkbox elements
    if (options.element.tagName.toUpperCase() !== 'INPUT' || options.element.type.toUpperCase() !== 'CHECKBOX') {
      setValidationValues(options, 'required', true);
    }
  });
  adapters.add('remote', ['url', 'type', 'additionalfields'], (options) => {
    const value = {
    ...
    $.each(splitAndTrim(options.params.additionalfields || options.element.name), (i, fieldName) => {
  ...
  adapters.add('password', ['min', 'nonalphamin', 'regex'], (options) => {
  ...
  adapters.add('fileextensions', ['extensions'], (options) => {
